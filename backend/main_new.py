...
# TULCA parameters from config
TULCA_PARAMS = config.get('tulca', {
    's_prime': 10,
    'v_prime': 3,
...
# Initialize data loader with domain
data_dir = Path(__file__).parent.parent / domain.data_dir
data_loader = DataLoader(str(data_dir), domain)

# Global state for TULCA model (initialized on first request)
tulca_model = None

# GeminiInterpreter with domain strategy
ai_interpreter = GeminiInterpreter(domain)
...
async def analyze_clusters(request: ClusterAnalysisRequest):
    ...
    try:
        ...
        # Get top important factors (domain-aware)
        top_factors = get_top_important_factors(contribution_matrix, domain, top_k=10)
        ...
        for factor in top_factors:
            ...
            # Statistical analysis (domain-aware)
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "data_loaded": data_loader.tensor_X is not None,
        "domain": domain.name,
